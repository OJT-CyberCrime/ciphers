...
interface CertificationOperationsProps {
...
const getStatusBadgeClass = (status: string) => {
  switch (status) {
    case 'pending':
      return { class: 'bg-yellow-200 text-yellow-800', label: 'P' };
    case 'resolved':
      return { class: 'bg-green-200 text-green-800', label: 'R' };
    case 'dismissed':
      return { class: 'bg-red-200 text-red-800', label: 'D' };
    case 'under investigation':
      return { class: 'bg-blue-200 text-blue-800', label: 'UI' };
    default:
      return { class: 'bg-gray-200 text-black', label: 'N/A' };
  }
};

export default function CertificationOperations({
  isAddingFolder,
  setIsAddingFolder,
  isEditingFolder,
  setIsEditingFolder,
  ...
  const handleAddFolder = async (e: React.FormEvent) => {
    ...
    try {
        ...
        .insert([
          {
            ...
            is_blotter: false,
            is_womencase: false,
            is_extraction: true // Set for certification/extraction
          }
        ])
        .select(`
          *,
      ...
      console.error('Error adding certificate folder:', error);
      toast.error(error.message || "Failed to create certificate folder");
    }
  };
  ...
  const handleEditFolder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error updating certificate folder:', error);
      toast.error(error.message || "Failed to update certificate folder");
    }
  };
  ...
  const handleArchiveFolder = async () => {
    ...
    try {
      ...
      toast.success("Certificate folder archived successfully");
      setDialogContent(null);
      setSelectedFolder(null);
    } catch (error: any) {
      console.error('Error archiving certificate folder:', error);
      toast.error(error.message || "Failed to archive certificate folder");
    }
  };
